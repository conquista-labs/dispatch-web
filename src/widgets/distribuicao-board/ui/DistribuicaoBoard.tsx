...
import { criarResolverInfoProtocolo, LegendaPrazo, useVisaoDistribuicao } from '@/entities/protocolo'
...
export const DistribuicaoBoard = () => {
  ...
  // A Distribuição é tela de gestão, que lê `GET /config` — as faixas da legenda vêm de lá.
  const { data: config } = useConfiguracao()
  const faixas = config
    ? { atencaoMinutos: config.faixaAtencaoMinutos, urgenteMinutos: config.faixaUrgenteMinutos }
    : null
  ...
  return (
    <div>
      ...
      <LegendaPrazo faixas={faixas} className="mt-3.5" />
      ...
      <div className="mt-4">
        {aba === 'conferente' && (
          <AbaPorConferente
            ...
            concluidosHojePorConferente={visao.concluidosHojePorConferente}
