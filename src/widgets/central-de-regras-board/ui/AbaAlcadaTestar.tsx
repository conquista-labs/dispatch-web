...
import {
  ...
  useRegrasAlcada,
  useTestarAlcada,
  type AvaliacaoAlcada,
} from '@/entities/regraAlcada'
...
export const AbaAlcadaTestar = ({ conferentes, tiposAto, equipes }: AbaAlcadaTestarProps) => {
  ...
  const caso = tipoAtoId
    ? { etapa, tipoAtoId, equipeId: equipeSelecionada === SEM_EQUIPE ? null : equipeSelecionada, prioridade }
    : null
    ...
    avaliacao.trilha.map((passo, indice) => {
      ...
      return (
        <div key={indice} className="mt-1 flex items-start gap-1.5">
          <span
            className={cn(
              'mt-1.5 block size-[5px] flex-none rounded-full',
              passo.efeito === 'Negado' ? 'bg-bad-fg' : 'bg-ok-fg',
            )}
          />
    ...
    <div>
      ...
      {resultado && (
        <>
          <div
            className={cn(
              'mt-2.5 rounded-[10px] border p-3.5',
              habilitados.length > 0 ? 'border-ok-border bg-ok-bg' : 'border-bad-border-2 bg-bad-bg',
            )}
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <span
                className={cn(
                  ...
                  habilitados.length > 0 ? 'text-ok-fg' : 'text-bad-fg',
                )}
              >
            ...
            <div className="mt-1 font-mono text-[11.5px] text-text-2">
              {tipoAtoId ? nomePorTipoAtoId.get(tipoAtoId) : ''} ·{' '}
              {equipeSelecionada === SEM_EQUIPE ? 'sem equipe' : nomePorEquipeId.get(equipeSelecionada)} ·{' '}
                ...
                <div key={a.conferenteId} className="border-t border-secondary py-2 first:border-t-0">
                  <div className="flex items-baseline justify-between gap-2">
                    ...
                    <span className="text-[11px] text-muted-foreground">
                      {nivelPorConferenteId.get(a.conferenteId)
                        ? `Analista ${NIVEL_LABEL[nivelPorConferenteId.get(a.conferenteId)!]}`
                        : ''}
                    </span>
                  </div>
                  {a.trilha.length > 0 ? (
                    ...
                    <div className="mt-1 text-[11.5px] text-muted-foreground">
                      nenhuma regra o alcança — liberado por padrão
                    </div>
                  )}
                </div>
              ))}
              {habilitados.length === 0 && (
                <div className="rounded-[8px] border border-dashed border-border p-4 text-center text-[12px] text-muted-foreground">
                  ninguém
                </div>
              )}
            </div>
                ...
                <div key={a.conferenteId} className="border-t border-secondary py-2 first:border-t-0">
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="text-[13px] font-medium text-text-3">
                      {nomePorConferenteId.get(a.conferenteId) ?? '—'}
                    </span>
                    <span className="text-[11px] text-bad-fg">
                      {a.motivo ? MOTIVO_ALCADA_LABEL[a.motivo] : 'barrado'}
                    </span>
                  </div>
                  {fraseDaTrilha(a)}
                </div>
      ...
      {tiposAto.length === 0 && (
        <p className="mt-2.5 text-[12.5px] text-muted-foreground">Cadastre um tipo de ato pra poder simular.</p>
      )}
    </div>
  )
}
